<?php
/**
 * Widget Name: Bean Portfolio Menu Widget
 *  
 *   
 * @package WordPress
 * @subpackage Spaces
...
add_action( 'widgets_init', 'reg_bean_portfolio_menu' );

// REGISTER WIDGET
function reg_bean_portfolio_menu() {
	register_widget( 'Bean_Portfolio_Menu_Widget' );
}
...
class Bean_Portfolio_Menu_Widget extends WP_Widget 
{
	...
	public function __construct() 
	{
		parent::__construct(
			...
			'Portfolio Menu', // NAME
			array( 'description' => __( 'A custom loop of portfolio posts.', 'bean' ), )
		);
	}
			...
			<li>
				<a title="<?php printf(__('Permanent Link to %s', 'bean'), get_the_title()); ?>" href="<?php the_permalink(); ?>">
					...
					<?php if( $loop == 'Most Views' ) { ?>
						<span class="subtext right"><?php echo bean_getPostViews(get_the_ID()); ?><?php _e( ' Views', 'bean' ); ?></span>	
					<?php } ?>
