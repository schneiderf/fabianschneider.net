...
//REVEAL CONTENT
if( get_theme_mod( 'reveal_content' ) == true ) { $reveal_content = 'reveal'; } else { $reveal_content = ''; } 
?>
...
<div class="row">
	...
	<section id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
			...
			<div class="six columns sidebar-right <?php echo $reveal_content ?>">
				...
				{ 
					if( get_theme_mod( 'reveal_content' ) == true ) { ?>
						<div class="after-post">
							...
							<?php if (get_comments_number()!= 0 ) { ?><div class="btn comments-btn"><span class="icon"></span><a href="<?php comments_link(); ?>"><?php _e( 'View Comments', 'bean' ); ?></a></div><?php } ?>
						</div><!-- END .after-post -->
					<?php } ?>
					...
					<?php if( get_theme_mod( 'about_author' ) == true ) { ?>
						...
						<div id="author-wrapper">
							...
							<h6><?php _e( 'Author', 'bean' ); ?></h6>

							<div class="fadein">

								<?php echo get_avatar( get_the_author_meta('user_email'), '80', '' ); ?>
								
								<h3 class="title"><?php _e( 'About the Author', 'bean' ); ?></h3>
								<p>
									<?php if (get_the_author_meta('description')) { the_author_meta('description'); } else { ?> 
									
									<?php _e( 'This author has not added a biography just yet. Meanwhile ', 'bean' ); the_author_meta('display_name'); _e( ' has contributed ', 'bean' ); the_author_posts(); _e( ' posts. ', 'bean' ); ?><a href="<?php echo get_author_posts_url(get_the_author_meta( 'ID' )); ?>"><?php _e( 'Click here', 'bean' );?></a><?php _e( ' to view them.', 'bean' );?>
					...
					<?php } //END  if( get_theme_mod( 'about_author' ) == true )
